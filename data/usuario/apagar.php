...
if(!empty($data->id)){
    ...
    if($usuario->apagar()){
        header("HTTP/1.0 200");
        echo json_encode(array("mensagem"=>"Usuario apagado com sucesso!"));
    }
    else{
        header("HTTP/1.0 400");
        echo json_encode(array("mensagem"=>"Não foi possivel apagar o usuario."));
    }
}
else{
    header("HTTP/1.0 400");
    echo json_encode(array("mensagem"=>"Você precisa passar todos os dados para apagar o usuario"));
}
?>