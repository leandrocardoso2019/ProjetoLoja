...
if(!empty($data->foto) && !empty($data->id)){
    ...
    if($usuario->alterarFoto()){
        header("HTTP/1.0 200");
        echo json_encode(array("mensagem"=>"Foto do usuario alterada com sucesso!"));
    }
    else{
        header("HTTP/1.0 400");
        echo json_encode(array("mensagem"=>"Não foi possivel alterar foto."));
    }
}
else{
    header("HTTP/1.0 400");
    echo json_encode(array("mensagem"=>"Você precisa passar todos os dados para alterar foto"));
}
?>